# 1 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\drivers\\mag3110.c"
# 1 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\pca10040\\s132\\ses//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\drivers\\mag3110.c"
# 1 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\drivers\\mag3110.h" 1
...
# 4 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\drivers\\mag3110.h" 2

# 4 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\drivers\\mag3110.h"
typedef enum {
 MAG_OS_16,
 MAG_OS_32,
 MAG_OS_64,
...
static void mag3110_write(uint8_t reg, uint8_t val) {
        i2cSend(0x0E,reg,&val,1);
}

static uint8_t mag3110_read(uint8_t reg) {
        static uint8_t ret = 0;
        i2cRecive(0x0E, reg, &ret,1);
 return ret;
}

void mag_init(mag_oversampling_t os) {

 mag3110_write(0x11, 0xA0);
 mag3110_write(0x10, 0x01 | (os<<3));
}
...
void mag_calibrate() {
 ...
 if (1 && (3 >= NRF_LOG_SEVERITY_INFO) && (NRF_LOG_SEVERITY_INFO <= 3)) { if (NRF_LOG_SEVERITY_DEBUG >= NRF_LOG_SEVERITY_INFO) { nrf_log_frontend_std_0(((NRF_LOG_SEVERITY_INFO) | m_nrf_log_app_logs_data_dynamic.module_id << 16), "Reading initial values"); } };
 do { while (nrf_log_frontend_dequeue()); } while (0);
 while (((mag_status = mag3110_read(0x00)) & 8) == 0)
  ;

 uint8_t x_msb = mag3110_read(0x01);
 uint8_t x_lsb = mag3110_read(0x02);
 uint8_t y_msb = mag3110_read(0x03);
 uint8_t y_lsb = mag3110_read(0x04);
 mag3110_read(0x05);
 mag3110_read(0x06);
 ...
 if (1 && (3 >= NRF_LOG_SEVERITY_INFO) && (NRF_LOG_SEVERITY_INFO <= 3)) { if (NRF_LOG_SEVERITY_DEBUG >= NRF_LOG_SEVERITY_INFO) { nrf_log_frontend_std_0(((NRF_LOG_SEVERITY_INFO) | m_nrf_log_app_logs_data_dynamic.module_id << 16), "Starting calibration"); } };
 do { while (nrf_log_frontend_dequeue()); } while (0);
 while (readings < 30*10) {
  while (((mag_status = mag3110_read(0x00)) & 8) == 0)
   ;

  if (1 && (3 >= NRF_LOG_SEVERITY_INFO) && (NRF_LOG_SEVERITY_INFO <= 3)) { if (NRF_LOG_SEVERITY_DEBUG >= NRF_LOG_SEVERITY_INFO) { nrf_log_frontend_std_0(((NRF_LOG_SEVERITY_INFO) | m_nrf_log_app_logs_data_dynamic.module_id << 16), "Value available\n"); } };
  do { while (nrf_log_frontend_dequeue()); } while (0);

  x_msb = mag3110_read(0x01);
  x_lsb = mag3110_read(0x02);
  y_msb = mag3110_read(0x03);
  y_lsb = mag3110_read(0x04);
  mag3110_read(0x05);
  mag3110_read(0x06);
  ...
  if (1 && (3 >= NRF_LOG_SEVERITY_INFO) && (NRF_LOG_SEVERITY_INFO <= 3)) { if (NRF_LOG_SEVERITY_DEBUG >= NRF_LOG_SEVERITY_INFO) { nrf_log_frontend_std_1(((NRF_LOG_SEVERITY_INFO) | m_nrf_log_app_logs_data_dynamic.module_id << 16), "Value read: %d\n", (uint32_t)(x)); } };
  do { while (nrf_log_frontend_dequeue()); } while (0);
 ...
 if (1 && (3 >= NRF_LOG_SEVERITY_INFO) && (NRF_LOG_SEVERITY_INFO <= 3)) { if (NRF_LOG_SEVERITY_DEBUG >= NRF_LOG_SEVERITY_INFO) { nrf_log_frontend_std_4(((NRF_LOG_SEVERITY_INFO) | m_nrf_log_app_logs_data_dynamic.module_id << 16), "Calibration complete: %d, %d, %d, %d", (uint32_t)(x_max), (uint32_t)(x_min), (uint32_t)(y_max), (uint32_t)(y_min)); } };
 do { while (nrf_log_frontend_dequeue()); } while (0);
...
# 137 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\drivers\\mag3110.c" 3 4
...
MAG_reading_t mag_read(){
 ...
 uint8_t y_msb = mag3110_read(0x03);
 uint8_t y_lsb = mag3110_read(0x04);
        uint8_t z_msb = mag3110_read(0x05);
 uint8_t z_lsb = mag3110_read(0x06);
