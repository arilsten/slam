...
# 41 "C:\\nRF5_SDK_15.0.0_a53641a\\components\\ble\\peer_manager\\peer_id.c" 2
...
# 1 "../../../../../../components/libraries/util/sdk_errors.h" 1
# 46 "C:\\nRF5_SDK_15.0.0_a53641a\\components\\ble\\peer_manager\\peer_id.h" 2
...
typedef struct
{
    uint8_t used_peer_ids[((7 + (256)) >> 3)];
    uint8_t deleted_peer_ids[((7 + (256)) >> 3)];
} pi_t;
...
void peer_id_init(void)
{
    ...
    pm_mutex_init(m_pi.used_peer_ids, 256);
    pm_mutex_init(m_pi.deleted_peer_ids, 256);
}


static pm_peer_id_t claim(pm_peer_id_t peer_id, uint8_t * mutex_group)
{
    pm_peer_id_t allocated_peer_id = 0xFFFF;
    if (peer_id == 0xFFFF)
    {
        allocated_peer_id = pm_mutex_lock_first_available(mutex_group, 256);
        if (allocated_peer_id == 256)
        {
            allocated_peer_id = 0xFFFF;
        }
    }
    else if (peer_id < 256)
    {
        
# 88 "C:\\nRF5_SDK_15.0.0_a53641a\\components\\ble\\peer_manager\\peer_id.c" 3 4
       _Bool 
# 88 "C:\\nRF5_SDK_15.0.0_a53641a\\components\\ble\\peer_manager\\peer_id.c"
            lock_success = pm_mutex_lock(mutex_group, peer_id);
        allocated_peer_id = lock_success ? peer_id : 0xFFFF;
    }
...
{
    pm_peer_id_t i = (prev_peer_id == 0xFFFF) ? 0 : (prev_peer_id + 1);
    for (; i < 256; i++)
    {
