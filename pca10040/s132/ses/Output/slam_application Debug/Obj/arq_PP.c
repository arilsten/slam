# 1 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\arq.c"
...
# 8 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\arq.c" 2
...
# 9 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\arq.c" 2
# 1 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\arq.h" 1
...
# 5 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\arq.h" 2


# 6 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\arq.h"
...
# 12 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\arq.c" 2
# 1 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\buffer.h" 1
...
static arq_connection_t connections[1];
...
void arq_init(void) {
  ...
  listening_task = 
# 64 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\arq.c" 3 4
                  0
# 64 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\arq.c"
                      ;
  uint8_t i=0;
  for(i=0;i<1;i++) {
    ...
    connections[i].mutex = xQueueCreateMutex( ( ( uint8_t ) 1U ) );
...
# 96 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\arq.c"
  ...
  xTaskNotifyWait(0xFFFFFFFF, 0x0, &sender, ( TickType_t ) 0xffffffffUL);
  ...
  connections[id].status = 2;
  uint8_t data = 3;
  network_send(sender, 1, &data, 1);
...
uint8_t arq_connect(arq_connection id, uint8_t remote_addr, void (*func)(uint8_t*, uint16_t), uint16_t timeout) {
  ...
  network_send(con->remote_address, 1, &data, 1);

  if(ulTaskNotifyTake(( ( BaseType_t ) 1 ), timeout) == 0) {
...
# 181 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\arq.c"
  ...
  if(total_len < 256 - con->send_buffer.len && 2*10 - con->segment_lengths.len >= 2) {
    uint16_t tmp = 50 - 6 -2;
    uint16_t remaining = total_len;
    buffer_append(&con->send_buffer, (uint8_t*) &len, 2);
    buffer_append(&con->send_buffer, data, len);

    while(remaining > 0) {
      tmp = remaining < 50 - 6 -2 ? remaining : 50 - 6 -2;
...
uint8_t arq_send_ack(arq_connection id, uint8_t sequence_number) {
  ...
  if(con->status != 2 || sequence_number > 127) {
    return 0;
  }
  ...
  return network_send(con->remote_address, 1, data, 2);
}
...
# 224 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\arq.c"
  ...
  uint8_t i;
  for(i=0;i<1;i++) {
    if(connections[i].remote_address == address && connections[i].status != 0) {
      con = &connections[i];
      id = i;
      break;
    }
...
# 255 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\arq.c"
  ...
  if(type == 0 || type == 4) {
    if(sequence == con->request_number) {
      if(type == 0) arq_reassembly(id, &data[2], len-2);
...
# 275 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\arq.c"
      ...
      con->timer = 0;
      con->timeout = 0;
      if(con->sequence_base == con->sequence_number) con->timer_started = 0;
    }
  }
...
# 310 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\arq.c"
    ...
    if(con->timer > 200) {
      ...
      while(i < resend_count) {
        ...
        network_send(con->remote_address, 1, data, len+2);
        ++i;
      }
      con->timer = 0;
    }
  }
  if( ((con->sequence_number-con->sequence_base) & 127) < 4 && con->send_buffer.head != con->send_buffer_window_end) {
    uint8_t data[50 - 6];
    uint16_t len;
    data[0] = 0;
...
void arq_reassembly(arq_connection id, uint8_t *data, uint16_t len) {
  ...
  if(con->receive_message_length == 0) {
    con->receive_message_length = data[0] | (data[1] << 8);
    len-=2;
    data+=2;

    if(con->receive_message_length > 100) {
      return;
    }
  }
