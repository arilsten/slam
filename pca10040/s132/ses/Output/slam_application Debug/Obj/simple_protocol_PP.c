# 1 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\simple_protocol.c"
...
# 2 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\simple_protocol.c" 2
# 1 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam\\communication\\simple_protocol.h" 1
...
static simple_message_t messages[1];
...
void simple_p_init(void (*cb)(uint8_t*, uint16_t)) {
  ...
  uint8_t i;
  network_set_callback(0, simple_p_reassembly);
  for(i=0;i<1;i++) {
...
uint8_t simple_p_send(uint8_t address, uint8_t *data, uint16_t length) {
  ...
  uint16_t offset = 0;
  uint8_t *part = pvPortMalloc(50 - 6);
  uint8_t part_number=0;
  uint8_t number_of_parts = (length/(50 - 6 -2)) + (length % (50 - 6 -2) != 0);
  while(remaining > 0) {
    tmp = remaining < (50 - 6 -2) ? remaining : (50 - 6 -2);
    part[0] = part_number++;
    part[1] = number_of_parts-1;
    memcpy(part+2, data+offset, tmp);
    network_send(address, 0, part, tmp+2);
    offset += tmp;
    remaining -= tmp;
  }
  vPortFree(part);
...
void simple_p_reassembly(uint8_t sender, uint8_t *data, uint16_t length) {
  ...
  if(id == 0xFF && free == 0xFF) return;
  else if(id == 0xFF && free != 0xFF) {
    id = free;
  }
  ...
  if(messages[id].num_received_bytes + (length-2) > 100) {
    messages[id].num_received_bytes = 0;
    messages[id].next_part = 0;
    messages[id].address = 0xFF;
    return;
